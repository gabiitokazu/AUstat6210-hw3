...
Because all modifications were in the same file, I made them using piping within the `raw_data` data frame and using the `mutate` function for each type of modification. For the modifications in more than one variable, I used the `across` function applied to a vector of them.
...
```{r q3, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}

q3 <- raw_data %>%
     select_if(is.factor)

# q3 <- with(raw_data,table(raw_data$Hospitalization.type,
#                          raw_data$Symptoms,
#                          raw_data$Outcome))
...
q5 <- arrange(raw_data, Date.of.outcome)
...
```{r q6, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
#NAO PODE ESQUECER DO BARPLOT HEIN
